...
build: generate
	go build $(LDFLAGS) -tags 'production' -o $(BINPATH)/ermintrude

debug: generate
	go build $(LDFLAGS) -tags 'debug' -o $(BINPATH)/ermintrude
	MONGO_URI=${MONGO_URI} HUMAN_LOG=1 BIND_ADDR=${BIND_ADDR} ENABLE_NEW_APP=${ENABLE_NEW_APP} $(BINPATH)/ermintrude
